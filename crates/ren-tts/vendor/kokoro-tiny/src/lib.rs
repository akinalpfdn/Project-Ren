...
impl TtsEngine {
    ...
    fn tokenize(&self, phonemes: &str) -> Vec<Vec<i64>> {
        // Kokoro expects the phoneme sequence to be bracketed by a single
        // padding token (id 0) on each end — this matches the reference
        // kokoro-onnx implementation. Without the bookends the model
        // interprets the first and last real phonemes as boundary
        // markers, which clips the opening and closing syllables of
        // every utterance.
        let mut tokens: Vec<i64> = Vec::with_capacity(phonemes.chars().count() + 2);
        tokens.push(0);
